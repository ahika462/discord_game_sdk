...
class Linc {
	...
	public static macro function xml(_lib:String, _target:String, _relative_root:String='../../../'):Array<Field> {
		...
		var _linc_include_path = Path.normalize(Path.join([ _linc_lib_path, 'haxe/discord/linc/$_target/linc_${_lib}.xml' ]));
		var _linc_lib_var = 'LINC_${_lib.toUpperCase()}_PATH';

		var _define = '<set name="$_linc_lib_var" value="$_linc_lib_path/"/>';
		var _import_path = '$${$_linc_lib_var}haxe/discord/linc/$_target/linc_${_lib}.xml';
		var _import = '<include name="$_import_path" />';

		Context.warning(StringTools.replace(_import_path, '$${$_linc_lib_var}', '$_linc_lib_path/'), _pos);
